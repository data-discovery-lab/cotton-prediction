...
def TPTF(X, Xtest, dims, rank, reg, reg_S, lr, lrS, max_iter, tol=0):
    ...
    assert (X[0].shape[0] == len(X[1]) and Xtest[0].shape[0] == len(
        Xtest[1])), "TPTF: rating and index must have the same length."
    ...
    while iter_count < max_iter:
        ...
        logger.info(
            'Training Loss: {:.2f} (change {:.2f}%). Testing RMSE: {:.4f}'.format(trn_loss, change_rate, test_rmse))
        tic = time.time()
...
if __name__ == "__main__":
    ...
    logger.info(
        'Settings: K = {}. reg = {}. regS = {}. lr = {}. lrS = {}'.format(rank, args.reg, args.regS, args.lr, args.lrS))
